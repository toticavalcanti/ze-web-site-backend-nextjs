...
async function loadDvd(identifier: string, { log = false } = {}) {
  ...
  if (log) {
    logger('📦 Campos do documento:', dvdDoc ? Object.keys(dvdDoc) : []);
    logger('📅 createdAt:', (dvdDoc as { createdAt?: unknown })?.createdAt);
    logger('📅 updatedAt:', (dvdDoc as { updatedAt?: unknown })?.updatedAt);
    logger('🔢 __v:', (dvdDoc as { __v?: unknown })?.__v);
  }
...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const dvdRecord = dvd as Record<string, unknown>;
    const cover = dvdRecord.cover as { related?: unknown } | undefined;

    console.log('✅ DVD encontrado:', dvdRecord.title);
    console.log('✅ Campos retornados:', Object.keys(dvdRecord));
